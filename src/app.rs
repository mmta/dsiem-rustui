...
pub fn App() -> Html {
    ...
    let alarm_id = search.replace('?', "");
    let state = use_async({
        let alarm_id = alarm_id.clone();
        async move {
            let url = location.protocol.clone() + "//" + &location.host;
            alarm::read(url, alarm_id).await
        }
    });
    ...
    html! {
        <div class={classes!("min-h-screen", "bg-white", "dark:bg-black")}>
            {
                if state.loading {
                    html! {<div class={classes!("loading", "dots")}>{"\u{00a0}\u{00a0}\u{00a0}"}</div>}
                } else {
                    html! {}
                }
            }
            {
                if let Some(data) = &state.data {
                    html! { <AlarmDetail alarm={data.clone()}/> }
                } else {
                    html! {}
                }
            }
            {
                if let Some(error) = &state.error {
                    html! { <p>{format!("Cannot read alarm ID: {}, Error: {}", alarm_id.clone(), error)}</p> }
                } else {
                    html! {}
                }
            }
    ...
    html! {
        <div>
            ...
            <table class={classes!("w-full","text-sm", "text-left", "text-gray-500", "dark:text-gray-400", "border-2", "dark:border-0")}>
                <thead class={classes!("text-xs","text-gray-700", "uppercase", "bg-gray-50", "dark:bg-gray-700", "dark:text-gray-400")}>
                    <tr>
